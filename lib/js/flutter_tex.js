...
function initTeXView(jsonData, engine) {
    ...
    // Create a render container to hold all content
    var renderContainer = document.createElement('div');
    renderContainer.id = 'tex-view-render-container';
    teXView.appendChild(renderContainer);
    
    // Add content to the render container instead of directly to teXView
    renderContainer.appendChild(createTeXView(jsonData));
...
function getTeXViewHeight(view) {
    // Get the render container which holds the actual content
    var renderContainer = document.getElementById("tex-view-render-container");
    if (!renderContainer) {
        // Fallback: measure the original teXView
        print("Fallback: measure the original teXView");
        return view.offsetHeight;
    }
    
    // Get the computed style for the container
    var style = window.getComputedStyle(renderContainer);
    
    // Get the full height including margins
    var totalHeight = renderContainer.offsetHeight;
    
    // Add margins (not included in offsetHeight)
    totalHeight += parseInt(style.marginTop || 0) + parseInt(style.marginBottom || 0);
    
    // Check if there's any overflow
    var scrollHeight = renderContainer.scrollHeight;
